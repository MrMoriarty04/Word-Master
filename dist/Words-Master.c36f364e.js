...
async function init() {
    ...
    function letterInsert(letter) {
        if (currentGuess.length < MAX_WORD_LENGTH) currentGuess += letter;
    ...
    async function submit() {
        ...
        for(let i = 0; i < MAX_WORD_LENGTH; i++)if (guessSplit[i] === wordSplit[i]) {
    ...
    document.addEventListener('keydown', function handleKeyPress(event) {
        if (!loadingDiv || done) return;
        const action = event.key;
        if (action === 'Enter') submit();
        else if (action === 'Backspace') backSpace();
        else if (isLetter(action)) letterInsert(action.toUpperCase());
    });
}
...
//# sourceMappingURL=Words-Master.c36f364e.js.map
